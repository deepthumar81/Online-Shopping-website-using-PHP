...
<body class="" style="background-image: url('Images/download.svg');">
    ...
    <form class=" flex justify-center">
        <div class="w-2/3 my-20 bg-hero-white shadow-lg shadow-slate-900 px-10 py-5 rounded-md ">
            <div class="space-y-5">
                ...
                <div class="border-b border-gray-500/10 pb-10 m-2">
                    ...
                    <div class="w-full">
                        ...
                        <div class="mt-2">
                            <select id="country" name="country" autocomplete="country-name" class="block w-full rounded-md border-0 py-1.5 px-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:max-w-xs sm:text-sm sm:leading-6">
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["edit"])) {
                        $name = $_POST["name"]; // Assuming you have a hidden input field for the name
                        // Perform the necessary update operation based on the provided name
                        // Example: $sql = "UPDATE laptops SET ... WHERE name = '$name'";
                        // Execute the SQL statement
                        // $conn->query($sql);
                        echo "Edit action triggered for $name"; // Replace this with the actual logic
                    }

                    // Handle the delete action
                    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete"])) {
                        $name = $_POST["name"]; // Assuming you have a hidden input field for the name
                        // Perform the necessary delete operation based on the provided name
                        $sql = "DELETE FROM laptops WHERE name = '$name'";
                        // Execute the SQL statement
                        $conn->query($sql);
                        echo "Delete action triggered for $name"; // Replace this with the actual logic
                    }

                    // Fetch laptop data from the database
                    $sql = "SELECT name, category, description, price, laptop_image FROM laptops LIMIT 3";
                    ...
                    ?>
                            <div class="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                                <div class="flex w-2/5">
                                    <div class="w-20">
                                        <img class="h-24" src="data:image/jpeg;base64,<?php echo base64_encode($image_content); ?>" alt="">
                                    </div>
                                    <div class="flex flex-col justify-between ml-12 flex-grow">
                                        <p name="name"><span class="font-bold text-sm"><?php echo $name; ?></span></p>
                                        <p><span class="text-red-500 text-xs"><?php echo $category; ?></span></p>
                                        <form method="POST" action="">
                                            <input type="hidden" name="name" value="<?php echo $name; ?>">
                                            <button type="submit" name="edit" id="edit" class="font-semibold hover:text-red-500 text-gray-500 text-xs">Edit</button>
                                        </form>
                                        <form method="POST" action="">
                                            <input type="hidden" name="name" value="<?php echo $name; ?>">
                                            <button type="submit" name="delete" id="delete" class="font-semibold hover:text-red-500 text-gray-500 text-xs">Delete</button>
                                        </form>
                                    </div>
                                </div>
                                ...
                                <div class="w-1/5 text-center pl-20">
                                    <p>$<?php echo $price; ?></p>
                                </div>
                            </div>
                    <?php
